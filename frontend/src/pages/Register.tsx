...
export default function Register() {
  ...
  const onSubmit = (data: RegisterFields) => {
    ...
    if (!strong) {
      setError('password', { type: 'validate', message: 'Password must be at least 8 characters and include letters and digits' })
      toast({ title: 'Password invalid', description: 'At least 8 characters, include letters and digits', variant: 'destructive' })
      return
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>Start using GreenBuildings SG</CardDescription>
        </CardHeader>
        <CardContent>
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div>
              ...
              <Input type="password" placeholder="At least 8 chars, letters and digits" {...register('password', { required: 'Please enter password', minLength: { value: 8, message: 'At least 8 characters' } })} />
              {errors.password && (<p className="text-xs text-red-600 mt-1">{errors.password.message as string}</p>)}
            </div>
            <div>
              <label className="text-sm font-medium">Confirm Password</label>
              <Input type="password" placeholder="Re-enter password" {...register('confirm', { required: 'Please confirm password' })} />
